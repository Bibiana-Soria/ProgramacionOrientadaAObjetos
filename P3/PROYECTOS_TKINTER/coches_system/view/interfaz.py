...
class Vista:
    ...
    def buscar_id(ventana,tipo,accion):
        ...
        if tipo=="Autos":
            if accion=="cambiar":
                Button(ventana,text="Buscar",command=lambda:Vista.cambiar_auto(ventana)).pack(pady=5)
            elif accion=="borrar":
                Button(ventana,text="Buscar",command=lambda:Vista.eliminar_auto(ventana)).pack(pady=5)
        elif tipo=="Camionetas":
            if accion=="cambiar":
